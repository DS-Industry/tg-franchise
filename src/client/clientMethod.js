        ...
        /*if (await reqMethod.searchActiveRequestOnTypeClient(client.id, type)){
            ...
            const addDescription = async (msg) =>{
                if (msg.chat.id === chatId) {
                    ...
                    await tgMethod.sendMessageWithRetry(chatId, `Теперь вы находитесь в ожидании ответа по вашему запросу. Все, что напишете сейчас, будет автоматически направлено администратору в качестве дополнительного комментария!\n Для общения по другому запросу переключитесь в меню всех обращений📩`);
